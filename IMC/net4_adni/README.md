# `net4_adni` – Network v04 for the ADNI Brain Dataset
...
here use the ADNI-specific `ADNIDataset` dataloader and run **5-fold
...
The ADNI dataset covers three classification tasks:
...
Training is launched with `python -m IMC.net4_adni.train`.  The script iterates
...
| `--dataset_path` | env default | Override `ADNI_LOCAL_DATASET_PATH` |
| `--label_csv_path` | env default | Override `ADNI_LABEL_CSV_PATH` |
| `--metadata_path` | env default | Override `ADNI_METADATA_PATH` |

---

## 2. Inference

Inference is launched with `python -m IMC.net4_adni.infer`.  The script:

- Loads the ADNI dataset (optionally filtered to specific folds via `--folds`).
...
```bash
python -m IMC.net4_adni.summarize_cv \
...
```bash
python -m IMC.net4_adni.summarize_cv ./logs/<timestamp>_5fold_cv
```
...
python -m IMC.net4_adni.summarize_cv ./logs/<run>
```
...
|---|---|---|
| `ADNI_LOCAL_DATASET_PATH` | ADNI brain MRI root dir | `--dataset_path` |
| `ADNI_LABEL_CSV_PATH` | ADNI label CSV | `--label_csv_path` |
| `ADNI_METADATA_PATH` | ADNI encoded metadata parquet | `--metadata_path` |
