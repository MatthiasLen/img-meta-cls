...
for test_fold in folds:
    ...
    sns.heatmap(cm, annot=True, fmt='d', xticklabels=cm_labels, 
...
sns.heatmap(cm_overall, annot=True, fmt='d', xticklabels=cm_labels, 
            yticklabels=cm_labels, cmap='Blues')
...
display_to_sort = {get_label_display_name(code): get_label_sort_order(code) 
...
sns.heatmap(pivot_precision, annot=True, fmt='.3f', cmap='YlGnBu',
            vmin=0, vmax=1, ax=axes[0], cbar_kws={'label': 'Precision'})
...
sns.heatmap(pivot_recall, annot=True, fmt='.3f', cmap='YlOrRd',
            vmin=0, vmax=1, ax=axes[1], cbar_kws={'label': 'Recall'})
...
sns.heatmap(pivot_f1, annot=True, fmt='.3f', cmap='RdYlGn',
            vmin=0, vmax=1, ax=axes[2], cbar_kws={'label': 'F1-Score'})
...
with open(report_path, 'w') as f:
    ...
    display_fold_df = fold_metrics_df[['fold', 'samples', 'accuracy', 
                                        'macro_f1', 'weighted_f1']].copy()
    ...
    f.write(f"- **Mean Accuracy**: {fold_metrics_df['accuracy'].mean():.4f} ± {fold_metrics_df['accuracy'].std():.4f}\n")
    f.write(f"- **Mean Macro F1**: {fold_metrics_df['macro_f1'].mean():.4f} ± {fold_metrics_df['macro_f1'].std():.4f}\n")
    f.write(f"- **Mean Weighted F1**: {fold_metrics_df['weighted_f1'].mean():.4f} ± {fold_metrics_df['weighted_f1'].std():.4f}\n")
    f.write(f"- **Best Fold (Accuracy)**: Fold {fold_metrics_df.loc[fold_metrics_df['accuracy'].idxmax(), 'fold']} ({fold_metrics_df['accuracy'].max():.4f})\n")
    f.write(f"- **Worst Fold (Accuracy)**: Fold {fold_metrics_df.loc[fold_metrics_df['accuracy'].idxmin(), 'fold']} ({fold_metrics_df['accuracy'].min():.4f})\n\n")
    ...
    display_class_df = class_stats_df[['class_display', 'precision_mean', 'precision_std',
                                       'recall_mean', 'recall_std', 
                                       'f1_mean', 'f1_std', 'total_support']].copy()
    display_class_df.columns = ['Class', 'Precision (mean)', 'Precision (std)',
    ...
    for _, row in class_stats_df.iterrows():
        ...
        if not class_fold_data.empty:
            ...
            f.write(f"- Precision: {class_fold_data['precision'].mean():.4f} ± {class_fold_data['precision'].std():.4f} "
                   f"(min: {class_fold_data['precision'].min():.4f}, max: {class_fold_data['precision'].max():.4f})\n")
            f.write(f"- Recall: {class_fold_data['recall'].mean():.4f} ± {class_fold_data['recall'].std():.4f} "
                   f"(min: {class_fold_data['recall'].min():.4f}, max: {class_fold_data['recall'].max():.4f})\n")
            f.write(f"- F1-Score: {class_fold_data['f1-score'].mean():.4f} ± {class_fold_data['f1-score'].std():.4f} "
