...
- Loads Duke labels from `LABEL_CSV_PATH` and **series-level** encoded metadata from `METADATA_PATH` (one row per series, not per slice).
...
- `LABEL_CSV_PATH`
- `METADATA_PATH` — series-level metadata parquet
- `LOCAL_DATASET_PATH`
...
- The series-level parquet (`METADATA_PATH` for XGBoost) is distinct from the slicewise parquet (`METADATA_PATH` for neural network experiments when generated with `--mode slice`); make sure to generate both if running all experiments.
