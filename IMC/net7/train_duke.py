"""Training entry point for Network v07 (PyramidPooling3DClassifier) on Duke.

This script trains :class:`~IMC.network07.PyramidPooling3DClassifier` on the
**Duke Liver MRI** dataset for **one fold per invocation**.  Run five processes
...
Or sequentially::
    ...
    python -m IMC.net7.train_duke --fold 0 --gpu 0
...
def init_weights(module: nn.Module) -> None:
    ...
    re-initialise only the non-pretrained layers.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for net7 Duke single-fold training.
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Test fold index (0–4).  Val fold = (fold + 1) %% 5.",
    )
    parser.add_argument(
        "--n_folds",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[2, 2, 2, 2],
        help="Block counts for ResNet backbone (e.g. 2 2 2 2).",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to ./logs/<timestamp>_net07_duke_5fold_cv.  "
...
def main(args: argparse.Namespace) -> None:
    """Train PyramidPooling3DClassifier on Duke for a single fold.

    1. Constructs train/val/test
       :class:`~IMC.data.duke_dataloader_3d.DukeLiverDataset3D` instances.
    ...
    4. Tests the best checkpoint and writes results to a JSON file.
    ...
    if args.base_log_dir is None:
        ...
        base_log_dir = os.path.join("./logs", f"{timestamp}_net07_duke_5fold_cv")
    else:
