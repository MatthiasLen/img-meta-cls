...
def load_fold_results(cv_dir, num_folds=5, eval_fold="evaluation"):
    ...
    """
    all_results = {"summary": [], "detailed": [], "metadata": []}
...
def generate_per_class_plots(class_stats_df, output_dir):
    ...
    for task in tasks:
        ...
        axes[0].set_title(f"{task.replace('label_', '')} - F1-Score by Class", fontsize=12, fontweight="bold")
        ...
        axes[1].set_title(
            f"{task.replace('label_', '')} - Precision vs Recall by Class", fontsize=12, fontweight="bold"
        )
    ...
    for idx, metric in enumerate(["Precision_mean", "Recall_mean", "F1-Score_mean"]):
        ...
        axes[idx].set_title(f"{metric.replace('_mean', '')} Heatmap", fontsize=12, fontweight="bold")
...
def generate_fold_comparison_plots(summary_df, output_dir):
    ...
    sns.heatmap(
        pivot_acc, annot=True, fmt=".3f", cmap="YlGnBu", vmin=0, vmax=1, ax=axes[0], cbar_kws={"label": "Accuracy"}
    )
    ...
    sns.heatmap(
        pivot_f1, annot=True, fmt=".3f", cmap="YlOrRd", vmin=0, vmax=1, ax=axes[1], cbar_kws={"label": "Macro F1"}
    )
    ...
    with open(report_path, "w") as f:
        ...
        for _, row in cv_stats_df.iterrows():
            ...
            for metric_key, metric_name in metrics_to_show:
                if f"{metric_key}_mean" in row:
                    f.write(
                        f"| {metric_name} | {row[f'{metric_key}_mean']:.4f} | "
        ...
        if not class_stats_df.empty:
            ...
            for task in class_stats_df["Task"].unique():
                ...
                task_data_display["Class"] = task_data_display["Label"].apply(lambda x: get_label_display_name(x))
                ...
                f.write(
                    f"**Best performing class**: {best_display} "
                    f"(F1: {best_class['F1-Score_mean']:.4f} ± {best_class['F1-Score_std']:.4f})\n\n"
                )
                f.write(
                    f"**Worst performing class**: {worst_display} "
...
Example usage:
  ...
  python summarize_cv.py logs/20260209_160303_5fold_cv --num_folds 5 --output_dir cv_summary
    ...
    parser.add_argument("cv_dir", type=str, help="Path to cross-validation directory containing fold_* subdirectories")
    parser.add_argument("--num_folds", type=int, default=5, help="Number of folds (default: 5)")
    parser.add_argument(
        "--output_dir", type=str, default=None, help="Directory to save summary outputs (default: cv_dir/cv_summary)"
    )
    parser.add_argument(
        "--eval_fold",
        type=str,
    ...
    print(
        f"  • {best_task['Task'].replace('label_', '')}: "
    ...
    print(
        f"  • {worst_task['Task'].replace('label_', '')}: "
