...
def generate_performance_plots(summary_df, output_dir, output_files):
    ...
    for bar, count in zip(bars, samples):
        ...
        plt.text(
            bar.get_x() + bar.get_width() / 2.0, height + max(samples) * 0.01, f"{int(count)}", ha="center", va="bottom"
        )
    plt.tight_layout()
...
def generate_report(summary_df, detailed_metrics_df, output_files, output_dir):
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"- **Best Performing Task (Accuracy)**: {summary_df.loc[summary_df['Accuracy'].idxmax(), 'Task']} ({summary_df['Accuracy'].max():.4f})\n"
        )
        f.write(
            f"- **Worst Performing Task (Accuracy)**: {summary_df.loc[summary_df['Accuracy'].idxmin(), 'Task']} ({summary_df['Accuracy'].min():.4f})\n"
        )
        f.write("\n")
...
Example usage:
  python evaluate_duke.py predictions.csv labels.csv --output_dir results
    ...
    parser.add_argument("prediction_csv", type=str, help="Path to the prediction CSV file.")
    parser.add_argument("label_csv", type=str, help="Path to the ground truth label CSV file.")
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    # Run evaluation
    summary_df, detailed_metrics_df, output_files = run_evaluation(pred_df, labels_df, args.output_dir)
