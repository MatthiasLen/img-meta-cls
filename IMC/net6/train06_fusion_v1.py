"""
Training script for Network v06 (FusionModelV1).

This script is adapted from the training script for Network v04.
"""
...
if __name__ == "__main__":
    from torch.utils.data import DataLoader
    from IMC.network06 import FusionModelV1
    ...
    experiment_name = f"fusion_model_v1_06_{args.backbone}"
    ...
    log_training_start(logger, config={"device": str(device), "batch_size": batch_size, "num_epochs": num_epochs, "dataset_version": "local", "model_version": "06", "impute": "yes", "learning_rate": lr, "checkpoint": args.ckpt})
    ...
    with capture_console_to_log(logger):
        ...
        model = FusionModelV1(metadata_input_dim=metadata_input_dim, num_classes_dict=cl_d)
        ...
        task_weights = [1.0] * len(cl_d.keys())
